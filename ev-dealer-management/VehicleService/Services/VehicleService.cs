...
public class VehicleService : IVehicleService
{
    ...
    public async Task<ReservationDto> ReserveVehicleAsync(int vehicleId, CreateReservationDto createDto)
    {
        var vehicle = await _context.Vehicles
            .Include(v => v.ColorVariants)
        ...
        if (vehicle.StockQuantity < createDto.Quantity)
        {
            throw new InvalidOperationException("Insufficient stock available");
        }

        // If color variant is specified, check its stock
        ColorVariant? colorVariant = null;
        if (createDto.ColorVariantId.HasValue)
        {
            colorVariant = vehicle.ColorVariants.FirstOrDefault(cv => cv.Id == createDto.ColorVariantId.Value);
            if (colorVariant == null)
            {
                throw new ArgumentException("Color variant not found");
            }
            if (colorVariant.Stock < createDto.Quantity)
            {
                throw new InvalidOperationException("Insufficient color variant stock available");
            }
        }

        // Calculate total price
        var totalPrice = vehicle.Price * createDto.Quantity;

        // Create reservation
        var reservation = new Reservation
        {
            VehicleId = vehicleId,
            ColorVariantId = createDto.ColorVariantId,
        ...
        // Reserve stock temporarily (decrease available stock)
        vehicle.StockQuantity -= createDto.Quantity;
        if (colorVariant != null)
        {
            colorVariant.Stock -= createDto.Quantity;
        }

        await _context.SaveChangesAsync();

        // Publish vehicle reserved event
        var vehicleReservedEvent = new VehicleReservedEvent
        {
            ...
            DealerId = vehicle.DealerId,  // Thêm DealerId
    ...
    public async Task<ReservationDto?> GetReservationByIdAsync(int id)
    {
        ...
        return new ReservationDto
        {
            ...
            VehicleName = reservation.Vehicle!.Model,
            ColorVariantId = reservation.ColorVariantId,
            ColorVariantName = reservation.ColorVariant?.Name,
            CustomerName = reservation.CustomerName,
            CustomerEmail = reservation.CustomerEmail,
            CustomerPhone = reservation.CustomerPhone,
            Notes = reservation.Notes,
    ...
    public async Task<PaginatedResult<ReservationDto>> GetReservationsAsync(ReservationQueryDto query)
    {
        ...
        {
            queryable = queryable.Where(r => r.CustomerEmail.Contains(query.CustomerEmail));
        }

        if (!string.IsNullOrWhiteSpace(query.CustomerPhone))
        {
            queryable = queryable.Where(r => r.CustomerPhone.Contains(query.CustomerPhone));
        }
        ...
        {
            queryable = queryable.Where(r => r.VehicleId == query.VehicleId.Value);
        }

        if (query.FromDate.HasValue)
        {
            queryable = queryable.Where(r => r.CreatedAt >= query.FromDate.Value);
        }

        if (query.ToDate.HasValue)
        {
            queryable = queryable.Where(r => r.CreatedAt <= query.ToDate.Value);
        }
        ...
        var reservations = await queryable
            ...
            .Select(r => new ReservationDto
            {
                ...
                VehicleName = r.Vehicle!.Model,
                ColorVariantId = r.ColorVariantId,
                ColorVariantName = r.ColorVariant!.Name,
